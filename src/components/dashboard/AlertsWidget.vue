      ...
      <q-list separator>
        <q-item v-for="alert in alerts" :key="alert.id" class="q-py-md">
           <q-item-section avatar top>
              ...
              <q-icon v-else-if="alert.type === 'info'" name="add_circle" color="deep-purple" size="sm" />
              <q-icon v-else-if="alert.type === 'warning'" name="warning" color="orange" size="sm" />
           </q-item-section>
           
           <q-item-section>
             <q-item-label class="text-weight-bold text-grey-9 q-mb-xs">{{ alert.title }}</q-item-label>
             <q-item-label caption class="text-grey-7">{{ alert.description }}</q-item-label>
             
             <div class="q-mt-sm" v-if="alert.time === 'ดูรายละเอียด'">
                <q-btn unelevated color="red-1" text-color="red-9" label="ดูรายละเอียด" size="xs" class="text-weight-bold" dense padding="xs md" />
             </div>
